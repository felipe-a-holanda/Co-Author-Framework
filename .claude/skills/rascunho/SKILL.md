---
description: Cria um rascunho de cena a partir de um prompt, lendo todo o atlas para garantir coerência dramatúrgica
---
...
5. Escrever o rascunho aplicando os princípios dramatúrgicos do framework:
   ...
   - Conflito latente baseado nas contradições dos personagens (ver `[[personagens]]`)
   - Coerência com o estado atual da narrativa (ver `[[continuidade]]`)
   - Tom e ambientação do universo (ver `[[universo]]`)
6. Salvar o rascunho em `{projeto}/rascunhos/<nome-do-arquivo>.md` com cabeçalho de metadados:
   ...
   **Posição sugerida:** [entre cena X e cena Y / no ato Z]
   ```
7. Apresentar o rascunho ao usuário com uma nota das escolhas dramatúrgicas feitas (por que esse conflito, por que essa estrutura de diálogo) e perguntar se aprova ou quer ajustes.
8. Se aprovado, perguntar se deseja promover o arquivo para `{projeto}/atos/` e atualizar `{projeto}/atlas/continuidade.md` com os novos eventos da cena.
